...
export function currentUser(prodDb, demoDb, c) {
  ...
  const user = dataDb.prepare('SELECT id, username, email, phone, language, role, lookahead_days, notification_level, display_name, avatar, created_at FROM users WHERE id = ?').get(s.user_id) || null
  if (!user) return null
...
export function handleDemoLogin(prodDb, c) {
  ...
  return { id: 'demo-user', username: 'demo', email: null, phone: null, language: 'auto', role: 'demo', notification_level: 'all', is_demo: true }
}
...
export async function createUser(db, { username, password, phone, role }) {
  ...
  return db.prepare('SELECT id, username, email, phone, language, role, lookahead_days, notification_level, display_name, avatar, created_at FROM users WHERE id = ?').get(id)
}

export function updateLanguage(db, userId, language) {
  db.prepare('UPDATE users SET language = ? WHERE id = ?').run(language, userId)
  return db.prepare('SELECT id, username, email, phone, language, role, lookahead_days, notification_level, created_at FROM users WHERE id = ?').get(userId)
}
...
export function updateNotificationLevel(db, userId, level) {
  db.prepare('UPDATE users SET notification_level = ? WHERE id = ?').run(level, userId)
}
