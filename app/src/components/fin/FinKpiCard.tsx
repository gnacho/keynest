...
interface FinKpiCardProps {
  ...
  unit?: string;
  decimals?: number;
  /** Delta vs período anterior en % */
  deltaPct?: number;
  /** true: subida = bueno (emerald). false: subida = malo (rose) — p. ej. gastos */
  deltaGoodWhenUp?: boolean;
  sub?: string;
  spark?: number[];
  sparkColor?: string;
  money?: boolean;
  /** Duración del count-up (0.4 s tras añadir gasto, 0.9 s al montar) */
  countDuration?: number;
  className?: string;
}

/**
 * KpiCard de Rentabilidad: como el compartido pero con delta con signo semántico
 * configurable (subida de gastos = rose aunque sea positiva).
 */
export default function FinKpiCard({
  icon: Icon,
  tone,
  label,
  value,
  ...
  const deltaColor = good ? '#10B981' : '#F43F5E';

  return (
    <div className={cn('card flex h-full flex-col gap-2 p-4', className)}>
      ...
      <div className="flex items-center gap-2">
        {deltaPct !== undefined && (
          <motion.span
            initial={reduce ? false : { opacity: 0, y: up ? 4 : -4 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.5, duration: 0.35, ease: EASE_OUT_QUART }}
            className="inline-flex items-center gap-0.5 rounded-full px-2 py-0.5 text-xs font-semibold"
            style={{ color: deltaColor, backgroundColor: good ? 'var(--em-chip-bg)' : 'var(--ro-chip-bg)' }}
          >
